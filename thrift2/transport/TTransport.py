...
class TTransportBase:
  ...
  def readAll(self, sz):
    # tag storyofus
    # self.read ==== .thrift2.transport.TTransport.TBufferedTransport
    buff = bytes()
    have = 0
    while (have < sz):
...
class TBufferedTransport(TTransportBase, CReadableTransport):
  ...
  def __init__(self, trans, rbuf_size=DEFAULT_BUFFER):
    self.__trans = trans
    self.__wbuf = BytesIO() # tag stroryofus
  ...
  def read(self, sz):
    ...
    # self.__trans  ==== thrift2.transport.TSocket.TSocket
    self.__rbuf = BytesIO(self.__trans.read(max(sz, self.__rbuf_size)))
    result = self.__rbuf.read(sz)
    return result
